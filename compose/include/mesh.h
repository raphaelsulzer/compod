...
public:
    ...
    void _save_region_mesh(const vector<Mesh::Face_index>& region, const string name);
    ...
    map<int, Mesh::Face_index> _polygon_to_face;
    map<int, Mesh::Vertex_index> _point_to_vertex;
    Mesh::Property_map<Mesh::Vertex_index, bool> _vertex_is_corner;
    Mesh::Property_map<Mesh::Vertex_index, CGAL::Color> _vcolor;
    map<Mesh::Face_index, int> _face_to_region;
    map<int, vector<Mesh::Face_index>> _region_to_faces;

    vector<vector<Mesh::Vertex_index>> _cycles_simplified;
    vector<vector<Mesh::Vertex_index>> _cycles_full;

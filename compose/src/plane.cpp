...
Point2 Plane::project(Point pt3){
...
void Plane::color_mesh_by_max_coord(Mesh& mesh){

    if(_max_coord == 0){
        mesh.add_property_map<Mesh::Vertex_index, CGAL::Color>("v:color",CGAL::red());
    }
    else if(_max_coord == 1){
        mesh.add_property_map<Mesh::Vertex_index, CGAL::Color>("v:color",CGAL::green());
    }
    else if(_max_coord == 2){
        mesh.add_property_map<Mesh::Vertex_index, CGAL::Color>("v:color",CGAL::blue());
    }
    else{
        cerr << "Invalid _max_coord" << endl;
        return;

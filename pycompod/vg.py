...
class VertexGroup:
    ...
    def extract_numbers_from_line(self, line):
        ...
        return numbers
    def _load_vg(self):
        primitives_file = self.input_file
        points = []
        normals = []
        groups = []
        planes = []
        ...
        return points, normals, groups, planes, colors
    ...
    def _process_vg(self):
        """
                ...
                :return:
                """
        points, normals, groups, planes, colors = self._load_vg()

        type = np.float64

        points = np.array(points).astype(type)
        colors = np.array(colors).astype(type)
        planes = np.array(planes).astype(type)
        normals = np.array(normals).astype(type)

        groups = [np.array(sublist, dtype=np.int32) for sublist in groups]
        ...
        if self.points_type == "samples":
            self.logger.info(
                "Sample a total of {} points on {} polygons".format(self.total_sample_count, len(self.polygons)))
        ...
        if self.export:
            ...
            self.plane_exporter.save_points_and_planes(point_filename=pt_file, plane_filename=plane_file,
                                                       points=self.points, normals=self.normals, groups=self.groups,
                                                       planes=self.planes, colors=self.plane_colors)
        ...
        if self.prioritise is not None:
            ...
            if not self.groups[
                       0].dtype == np.int32:  # this line is only here for the rare case that all group arrays are of the same size, then numpy sets the dtype of each individual array to object
            ...
            if not self.hull_vertices[
                       0].dtype == np.int32:  # this line is only here for the rare case that all hull_vertices arrays are of the same size, then numpy sets the dtype of each individual array to object
        ...
        if self.export:
            ...
            self.plane_exporter.save_points_and_planes(plane_filename=plane_file, point_filename=pt_file,
                                                       points=self.points, normals=self.normals, groups=self.groups,
                                                       planes=self.planes, colors=self.plane_colors)
